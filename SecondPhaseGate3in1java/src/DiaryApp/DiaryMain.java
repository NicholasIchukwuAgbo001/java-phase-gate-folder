    ...
    while (true) {
        if (diary == null) {
            ...
            if (choice == 1) {
                ...
                for (Diary diarry : diaries) {
                    diary = diarry;
                    if (diarry.getUsername().equals(username)) {
                        System.out.println("Diary found.");
                        System.out.print("\nEnter password to unlock diary: ");
                        String unlockPassword = userInputCollection.nextLine();
                        try {
                            diary.unlockDiary(unlockPassword);
                ...
                for (Diary diaryHolder : diaries) {
                    if (diaryHolder.getUsername().equals(username) && diaryHolder.getPassword().equals(password)) {
                        diaries.remove(diaryHolder);
